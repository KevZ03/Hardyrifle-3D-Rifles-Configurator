...
- They can **submit the build** to Hardy. Submission asks for contact details, fires a six-digit code to their email to verify the address, and then sends them a confirmation email plus a region-routed staff notification with the build sheet attached.
...
- **Supabase Storage** — three buckets: a public bucket for 3D model files, a public bucket for build screenshots, and a private bucket for PDF build sheets.
...
- **Resend** — used by an older order-confirmation flow that has since been deprecated. Brevo is the active path.
...
- **@hello-pangea/dnd** — powers the five-column Kanban on the staff board.
...
**Deployment**
**Vercel** — hosting and deployment for the full Next.js application, 
including serverless API routes for email and PDF generation.
...
**Submission flow.** A submission endpoint exposes two actions:
...
2. `action: 'verify_code'` — checks the code, upserts the customer record, enforces rate limiting per customer per day, generates an order reference (`RB-YYYY-XXXX`), inserts the build into `rifle_builds`, uploads the PDF to a private storage bucket, and fires two emails: a customer confirmation and a region-routed staff notification.
...
**Staff manage board.** The staff board is gated behind authentication. It loads every row in `rifle_builds`, joins the `customers` table, and renders five drag-and-drop columns. Reordering or stage changes hit a build management endpoint with a `columnOrders` payload, which updates `stage` and `position` for every affected row in parallel. The same endpoint also handles inline edits to the build spec, customer info, dealer, and notes.
